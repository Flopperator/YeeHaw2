...
extension Decodable{
    //Turn name back to normal String
    init(fromDictionary: Any)
    throws{
...
extension String {
    func tokenizeString() -> [String]{
        ...
        let trimmed = String(self.filter{!"\n\t\r".contains($0)}) //Regex to remove spaces or new line characters
        
        for(index, _) in trimmed.enumerated(){
            let prefixIndex = index + 1
            let substringPrefix =
            String(trimmed.prefix(prefixIndex)).lowercased() //puts it into the array and lowercases everything
