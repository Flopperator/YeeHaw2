...
class ChatService:ObservableObject{
 @Published var isLoading = false
    ...
    DocumentReference{
        return
        messages.document(senderId).collection("messages").document(recipientId)
    }
    func loadChats(){
        self.chats = []
        self.isLoading = true
    ...
    func sendMessage(message: String, recipientId: String, recipientProfile: String, recipientName: String, onSuccess:
        ...
        let chat = Chat(messgeId: messageId, textMessgae: messageId, profile: senderProfile, photoUrl: "", sender: senderId, username: senderUsername, timestamp: Date().timeIntervalSince1970, isPhoto: false)
    ...
    func sendPhotoMessage(imageData: Data, recipientId: String, recipientProfile: String, recipientName: String, onSuccess:
                          @escaping()-> Void, onError: @escaping(_ error: String)-> Void){
        guard let senderId = Auth.auth().currentUser?.uid else{
            return
        }
    ...
    func getChats(userId: String, onSuccess: @escaping([Chat])-> Void, onError: @escaping(_ error: String)-> Void, newChat: @escaping(Chat)-> Void,
    ...
    func getMessages(onSuccess: @escaping([Message])-> Void, onError: @escaping(_ error: String)-> Void, newMessage: @escaping(Message)-> Void,
                     listener: @escaping(_ listenerHandle: ListenerRegistration )-> Void){
